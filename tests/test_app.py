...
def test_api_get_versions():
	with flaskapp.app.test_request_context("/"):
		...
		assert set(flaskapp.api_get_versions().values()) == {sklearn.__version__, python_version(), ie_bike_model.__version__}
...
def test_api_train_and_persist():
	with flaskapp.app.test_request_context("/train_and_persist"):
		...
		assert isinstance(flaskapp.api_train_and_persist().get("persisted-model-parameters"), dict)
